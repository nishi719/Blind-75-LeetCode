...
I'll try to explain this with an example try to Imagine you're at a party and you're trying to find two people whose ages add up to a specific number. The brute force method would be like going to each person one by one and then for each of them checking their age against every other person's age in the room. It's thorough, but time-consuming.
...
However, this simplicity comes at a cost. As our input size grows, the number of comparisons we need to make grows quadratically. With small arrays, this might not be noticeable, but as we approach the upper limit of 10 
4 elements, the time taken could become significant.
...
-> Time Complexity Analysis: Worst case and Average case: O(n^2)
...
Best case: O(1) If the first two elements we check are the solution, we'll find it immediately.

Space Complexity Analysis: O(1) or constant space
...
1.If the current sum is less than the target, we need a larger sum. Since the array is sorted, moving the left pointer to the right will give us a larger number.
2.If the current sum is greater than the target, we need a smaller sum. Moving the right pointer to the left will give us a smaller number.
3.This approach is more efficient than the brute force method because we're making an informed decision at each step, effectively eliminating a large portion of the search space.
...
Overall time complexity: O(nlogn), dominated by the sorting step
...
We start with an empty hash table (complementMap). This will store numbers we've seen and their indices.
